...
/* Skin Editor больше не рисовалка по пикселям — игрок загружает готовую
   ...
   3) прогоняется через настоящую нейросеть (nsfwjs, модель InceptionV3 —
      бесплатная, с открытым кодом, https://github.com/infinitered/nsfwjs,
      самая точная из трёх моделей, встроенных в пакет — см. getModel()
      ниже), которая распознаёт голых людей / порнографию / хентай, и через
   ...
   nsfwjs целиком локальная: веса всех трёх моделей лежат прямо в npm-пакете
   (InceptionV3 — ~29 МБ, node_modules/nsfwjs/dist/models/inception_v3),
   сеть наружу не ходит — значит это можно спокойно назвать «бесплатной
   нейросетью» без скрытых платных вызовов и без утечки загруженных фото
   куда-то ещё. Она честно ошибается реже, чем прежний чисто цветовой
   фильтр (тот проверял только долю «телесных» тонов пикселей и путал с
   ней, например, портрет крупным планом) — но всё равно не идеальна:
   возможны и пропуски (необычное освещение/стиль), и отказ по ошибке на
   пограничных фото. Владелец по-прежнему может удалить любой скин вручную
   (/skins/remove), а /owner/rescanSkins — прогнать этой же (обновлённой)
   моделью всё, что уже опубликовано, — это подстраховка, а не единственный
   рубеж. */
'use strict';
...
/* tfjs-node вместо чистого tfjs: та же самая библиотека, но регистрирует
   нативный (C++, скомпилированный при npm install) бэкенд вместо
   JS-эмуляции — сама nsfwjs предупреждает об этом в консоли при старте.
   С InceptionV3 (см. getModel() ниже) разница критична: без нативного
   бэкенда одна проверка картинки занимала ~25 секунд (замерено), что для
   формы публикации скина неприемлемо — с ним должно быть на два порядка
   быстрее. API идентичен @tensorflow/tfjs, просто другой пакет. */
...
/* Пороги нейросети (вероятность класса 0..1). Начинали с 0.5/0.5/0.85, потом
   ужесточили до 0.15/0.15/0.5 — но всё ещё пропускало голое тело на тёмных/
   плохо освещённых кадрах (сеть на таких снимках заметно недоуверена во всех
   классах сразу, включая Porn/Hentai — не только Neutral проседает). Раз
   пропуски продолжаются — режем ещё сильнее, до порогов, на которых уже
   почти любой самый слабый сигнал по голому телу считается нарушением.
   Осознанно: это резко поднимает долю ложных срабатываний на пограничных
   нормальных фото (яркая кожа крупным планом, купальники, art с обнажёнкой
   без порно-контента и т.п.) — так и должно быть, здесь предпочтение отдано
   стороне «лучше отклонить нормальное», а не «лучше пропустить голое тело».
   Комбинированный порог — тоже равновзвешенный и низкий: тройка небольших
   по отдельности чисел (Porn/Hentai/Sexy) всё равно должна флагаться. */
const NSFW_PORN_LIMIT = 0.06;
const NSFW_HENTAI_LIMIT = 0.06;
const NSFW_SEXY_LIMIT = 0.25;
const NSFW_COMBINED_LIMIT = 0.12;   // porn + hentai + sexy*0.5

/* Из трёх моделей, встроенных в сам пакет nsfwjs (MobileNetV2, MobileNetV2Mid,
   InceptionV3 — все три уже лежат локально в node_modules, никуда наружу
   ходить не нужно), InceptionV3 самая точная (по данным самого nsfwjs —
   ~93% против ~90% у MobileNetV2, которую использовали раньше): более
   глубокая сеть, вход 299×299 вместо 224×224. За это платим более долгим
   прогревом при старте сервера и более медленной (но по-прежнему разовой,
   на одну публикацию скина, а не в реальном времени) проверкой одной
   картинки — ради заметно меньшей доли пропусков это оправдано. */
// ---------- нейросеть: грузим один раз на весь процесс ----------
let modelPromise = null;
function getModel() {
  if (!modelPromise) {
    modelPromise = nsfwjs.load('InceptionV3').catch((e) => {
