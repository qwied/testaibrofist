...
(function () {
  ...
  // Яркость цвета — по ней понимаем, не сливается ли вещь с силуэтом.
  function lum(hex) {
  ...
  /* Цвет силуэта задаёт игра, и часть вещей каталога (чёрные очки, пиратская
     шляпа, подтяжки) совпадала с ним почти в тон — на фигуре от аксессуара
     оставались только светлые блики, и вещь выглядела набором случайных
     пятен. Здесь такой цвет отодвигаем по яркости, пока вещь не станет
     видна; на контрастном силуэте цвет остаётся исходным. */
  function pop(c, base) {
    var lb = lum(base), dir = lb < 140 ? 1 : -1, out = c;
    for (var i = 0; i < 14 && Math.abs(lum(out) - lb) < 34; i++) out = shade(out, dir * 9);
    return out;
  }
  ...
  function head(id, c) {
    ...
    switch (id) {
      ...
      case 'h_ears':
        return [P('M14 30 L26 -20 L48 12 Z', c), P('M86 30 L74 -20 L52 12 Z', c),
                P('M22 24 L29 -6 L41 13 Z', l), P('M78 24 L71 -6 L59 13 Z', l)];
      case 'h_antenna':
        return [P('M50 6 q2 -30 12 -42', null, o, 9),
                P('M50 6 q2 -30 12 -42', null, c, 5),
                C(62, -36, 12, o), C(62, -36, 9, l)];
      case 'h_hair':
      ...
      case 'h_headband':
        return [P('M88 40 q14 6 16 20 q-13 0 -20 -10 z', d),
                P('M86 44 q7 11 5 22 q-11 -5 -13 -15 z', d),
                R(2, 32, 96, 14, 7, c),
                P('M6 39 h88', null, l, 4)];
      case 'h_bow':
        return [P('M64 20 q-2 15 -8 23 l10 4 q6 -12 6 -23 z', d),
                P('M76 20 q2 15 8 23 l-10 4 q-6 -12 -6 -23 z', d),
                P('M70 16 q-18 -18 -30 -6 q-5 11 8 14 q13 3 22 -8 z', c),
                P('M70 16 q18 -18 30 -6 q5 11 -8 14 q-13 3 -22 -8 z', c),
                C(70, 16, 8, d)];
      case 'h_flower':
        return [P('M36 16 q-3 -12 0 -22', null, '#16a34a', 4),
                C(36, -14, 8, c), C(45, -6, 8, c), C(36, 2, 8, c), C(27, -6, 8, c),
                C(36, -6, 5.5, '#fde047')];
      case 'h_propeller':
        return [P('M18 32 A32 32 0 0 1 82 32 Z', c),
                ...
                C(50, -12, 7, l)];
      case 'h_unicorn':
        return [P('M40 12 L50 -46 L60 12 Z', c),
                P('M43 0 L57 0', null, o, 3),
                P('M45 -12 L55 -12', null, o, 3),
                P('M47 -26 L53 -26', null, o, 3)];
      default: return [];
    }
  }
  ...
  function face(id, c) {
    ...
    switch (id) {
      ...
      case 'f_war':
        return [P('M13 50 q16 -6 29 2', null, c, 8),
                P('M15 66 q16 -6 27 2', null, c, 8),
                P('M87 50 q-16 -6 -29 2', null, c, 8),
                P('M85 66 q-16 -6 -27 2', null, c, 8)];
      case 'f_freckles':
      ...
      case 'f_pipe':
        return [P('M52 74 q16 6 23 10', null, c, 6),
                E(79, 88, 9, 8, c),
                C(87, 66, 4, '#cbd5e1'), C(83, 54, 3, '#e5e7eb')];
      case 'f_lolli':
    ...
    switch (id) {
      case 'k_cape':
        return [P('M4 ' + (T+2) + ' L96 ' + (T+2) + ' L134 ' + (H+6) +
                ...
                R(2, T-2, 96, 16, 8, l)];
      case 'k_wings':
        return [P('M16 ' + (T+14) + ' q-52 -32 -70 24 q-10 40 8 56 q24 -40 68 -22 z', c, d, 5),
                P('M84 ' + (T+14) + ' q52 -32 70 24 q10 40 -8 56 q-24 -40 -68 -22 z', c, d, 5),
                P('M14 ' + (T+46) + ' q-30 -10 -50 14', null, d, 4),
                P('M86 ' + (T+46) + ' q30 -10 50 14', null, d, 4)];
      case 'k_jetpack':
      ...
      case 'k_bag':
        return [R(-34, T+18, 168, 126, 26, c),
                R(-34, T+18, 168, 28, 14, d),
                R(-34, T+72, 168, 10, 0, d),
                C(-16, T+100, 8, l), C(116, T+100, 8, l)];
      case 'k_tail':
        return [P('M66 ' + (H-38) + ' q74 14 58 -80 q-18 62 -58 50 z', c),
                P('M84 ' + (H-46) + ' q44 6 38 -48', null, l, 5)];
      case 'k_shell':
      ...
      case 'k_butterfly':
        return [P('M24 ' + (T+14) + ' q-48 -32 -62 6 q-6 22 20 22 q28 0 42 -18 z', c, d, 5),
                P('M76 ' + (T+14) + ' q48 -32 62 6 q6 22 -20 22 q-28 0 -42 -18 z', c, d, 5),
                P('M26 ' + (T+46) + ' q-38 -4 -46 22 q-5 21 17 19 q22 -3 29 -27 z', d),
                P('M74 ' + (T+46) + ' q38 -4 46 22 q5 21 -17 19 q-22 -3 -29 -27 z', d),
                C(-10, T+28, 6, l), C(110, T+28, 6, l),
                C(-4, T+58, 4, l), C(104, T+58, 4, l)];
      case 'k_bat':
        return [P('M20 ' + (T+10) + ' q-44 -24 -70 10 l13 5 l-10 12 l17 3 l-8 14 l18 -2 ' +
                  'l-1 13 q32 -8 41 -40 z', c),
                P('M80 ' + (T+10) + ' q44 -24 70 10 l-13 5 l10 12 l-17 3 l8 14 l-18 -2 ' +
                  'l1 13 q-32 -8 -41 -40 z', c)];
      case 'k_quiver':
        return [P('M92 ' + (T+50) + ' q-46 -16 -54 -46', null, d, 7),
                L(96, T+46, 96, T-6, '#d4a017', 4),
                L(108, T+46, 108, T-18, '#d4a017', 4),
                L(120, T+46, 120, T+2, '#d4a017', 4),
                P('M92 ' + (T-6) + ' L96 ' + (T-18) + ' L100 ' + (T-6) + ' Z', '#94a3b8'),
                P('M104 ' + (T-18) + ' L108 ' + (T-30) + ' L112 ' + (T-18) + ' Z', '#94a3b8'),
                P('M116 ' + (T+2) + ' L120 ' + (T-10) + ' L124 ' + (T+2) + ' Z', '#94a3b8'),
                R(84, T+40, 40, 112, 14, c), R(84, T+40, 40, 18, 9, d)];
      case 'k_sword':
        return [C(120, T-54, 8, '#d4a017'),
                R(113, T-48, 14, 42, 7, '#8b5a2b'),
                P('M99 ' + (T-8) + ' L141 ' + (T-8) + ' L139 ' + (T+4) + ' L101 ' + (T+4) + ' Z', '#d4a017'),
                P('M112 ' + (T+6) + ' L128 ' + (T+6) + ' L120 ' + (T+156) + ' Z', c),
                P('M120 ' + (T+14) + ' L120 ' + (T+134), null, l, 3)];
      case 'k_shield':
        return [C(50, T+90, 78, c), C(50, T+90, 78, null, d, 6),
                C(50, T+90, 54, null, l, 4), C(50, T+90, 12, d), C(46, T+84, 4, l)];
      case 'k_axe':
        return [P('M128 ' + (T-34) + ' L104 ' + (T+140), null, '#8b5a2b', 12),
                P('M102 ' + (T-56) + ' q42 -10 47 28 q4 26 -16 36 ' +
                  'q-23 -7 -30 -31 q-7 -21 -1 -33 z', c),
                P('M139 ' + (T-40) + ' q10 18 3 38', null, l, 5),
                R(118, T-50, 13, 13, 4, '#8b5a2b')];
      case 'k_balloon':
        return [P('M114 ' + (T-30) + ' q10 44 0 88 q-5 18 -20 24', null, d, 3),
                E(114, T-70, 26, 33, c),
                E(123, T-81, 7, 11, l),
                P('M108 ' + (T-40) + ' l12 0 l-6 9 z', d)];
      case 'k_parrot':
        return [P('M100 ' + (T+8) + ' l-8 42 l14 2 l6 -40 z', '#b91c1c'),
                E(116, T-26, 19, 28, '#e04141'),
                E(116, T-22, 10, 19, '#16a34a'),
                C(116, T-46, 11, '#e04141'),
                P('M125 ' + (T-50) + ' q12 2 10 10 q-6 4 -12 0 z', '#fbbf24'),
                C(118, T-48, 2.5, '#111827')];
      case 'k_fairy':
        return [E(4, T+26, 38, 20, c, d, 4), E(96, T+26, 38, 20, c, d, 4),
                E(0, T+64, 28, 15, c, d, 3), E(100, T+64, 28, 15, c, d, 3),
                E(6, T+24, 23, 11, l), E(94, T+24, 23, 11, l)];
      case 'k_skateboard':
        return [C(18, H + 32, 8, '#f8fafc'), C(82, H + 32, 8, '#f8fafc'),
                C(18, H + 32, 3.5, d), C(82, H + 32, 3.5, d),
                P('M8 ' + (H+10) + ' L92 ' + (H+10) + ' q15 0 15 8 q0 8 -15 8 L8 ' + (H+26) +
                  ' q-15 0 -15 -8 q0 -8 15 -8 z', c),
                P('M-2 ' + (H+18) + ' h104', null, d, 3)];
      default: return [];
    }
  }
  ...
  function parts(skin, byId, bodyColor) {
    ...
    var col = function (slot) {
      ...
      return pop((it && it.v) ? it.v : DEF[slot], base);
    };
    return {
      base: base,
